document.getElementById('setLimit').addEventListener('click', function() {
    ...
    if (enteredCode === '1234') { // Exemple de code parental, à personnaliser
        if (timeLimit) {
            ...
            // Sauvegarder la limite de temps et l'heure de fin dans chrome.storage
    ...
    const interval = setInterval(function() {
        chrome.storage.local.get(['endTime'], function(result) {
            ...
            if (timeRemaining > 0) {
                ...
                // Afficher immédiatement le pop-up de blocage quand le temps est écoulé
...
// Fonction pour afficher le pop-up de blocage lorsque le temps est écoulé
...
chrome.storage.local.get(['endTime'], function(result) {
    ...
    const endTime = result.endTime || 0;
    if (endTime > now) {
        startCountdown(); // Redémarrer le compteur si une limite est déjà définie
    }
});
