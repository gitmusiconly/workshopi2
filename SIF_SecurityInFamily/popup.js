...
// Ajout de la logique pour le bouton "Signaler comme inapproprié"
document.getElementById('reportButton').addEventListener('click', function() {
    // Afficher un message de confirmation avec fond bleu
    displayMessage("Merci pour votre signalement !", "Signalement", "blue");
});

// Ajout de la logique pour le bouton "Désactiver"
document.getElementById('disableBlock').addEventListener('click', function() {
    let enteredCode = document.getElementById('parentCode').value;

    // Vérification du code parental pour désactiver le blocage
    if (enteredCode === '1234') { // Code parent à personnaliser
        // Supprimer la clé 'endTime' pour désactiver le blocage
        chrome.storage.local.remove('endTime', function() {
            displayMessage("Blocage désactivé.", "Confirmation", "green");
        });
    } else {
        displayMessage("Code parental incorrect. Impossible de désactiver le blocage.", "Erreur", "red");
    }

    // Vider le champ du code parental après utilisation
    document.getElementById('parentCode').value = '';
});
